...
def mediaSubjectView(request):
    subjects = Subject.objects.all()
    mediaMajor = subjects.filter(major__name="미디어학부")

    return render(request, 'media.html', {'mediaMajor': mediaMajor})

def majorSubjectView(request,pk):
    subjects = Subject.objects.all()
    Major = subjects.filter(pk=pk)
